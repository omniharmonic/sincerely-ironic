/**
 * The mark. Two silhouettes — one white, one black — on the slick. The
 * gradient is CSS (so it drifts with everything else that is rainbow on the
 * site); the silhouettes are the supplied paths, untouched.
 */
  ...
  return (
    <span className={`logo slick-bg ${className}`} role="img" aria-label={title}>
      <svg viewBox={VIEWBOX} className="logo__figures" aria-hidden="true" focusable="false">
        <path d={FIGURE_B} fill="#ffffff" />
        <path d={FIGURE_A} fill="#0d0d0d" />
      </svg>
    </span>
  );
}
...
export const LOGO_PATHS = { viewBox: VIEWBOX, white: FIGURE_B, black: FIGURE_A };

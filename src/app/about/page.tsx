...
export default function AboutPage() {
  return (
    <article style={{ padding: "clamp(40px, 7vw, 110px) var(--gutter) 0" }}>
      {/* The page is two columns of held measure, so past a point it stops
          growing. Centring what is left keeps the empty space either side of
          the text rather than stranding it all down one edge. */}
      <div className="mx-auto w-full max-w-[1180px]">
        <h1
          className="display text-[clamp(44px,9vw,150px)]"
          style={{ ["--wdth" as string]: 118 }}
        >
          <T s={about.title.sincere} i={about.title.ironic} />
        </h1>

        {/* Both columns are held to a measure. The questions used to take
          `1fr`, so on a wide screen they had thirteen hundred pixels to
          themselves and the answers read as the page rather than an aside. */}
        <div className="mt-14 grid gap-14 lg:grid-cols-[minmax(0,58ch)_minmax(0,26rem)] lg:gap-20">
          ...
          <aside className="lg:border-l lg:border-line lg:pl-10">
            <h2
              className="display text-[clamp(24px,2.6vw,36px)]"
              style={{ ["--wdth" as string]: 125 }}
            >
              <T
                s={about.questionsHeading.sincere}
                i={about.questionsHeading.ironic}
              />
            </h2>
            ...
            <dl className="mt-6 flex flex-col">
              {about.questions.map((q, i) => (
                <div key={i} className="border-t border-line py-6">
                  ...
                  <dd
                    className="display mt-3 text-[clamp(26px,2.4vw,36px)] leading-[1.05]"
